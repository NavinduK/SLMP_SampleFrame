...
namespace SLMP_SampleFrame.Services
{
    public class PlcReader : PlcConnection
    {
        ...
        // ===============================
        public void WriteSingleWBit(int address, int bit, bool on)
        {
            // Read current word
            ushort current = ReadSingleW(address);
            ushort newValue;
            ...
            try
            {
                ...
                Console.WriteLine($"✅ W{address}.{bit} written {(on ? "ON" : "OFF")} successfully");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error writing W{address}.{bit}: {ex.Message}");
            }
        }

        // ===============================
        // Print all bits in a word
        // ===============================
        public void PrintBits(ushort value, int address)
        {
            Console.WriteLine($"\nW{address} = 0x{value:X4} ({value})");
